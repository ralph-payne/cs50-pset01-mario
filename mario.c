// Program which draws triangles of height h. 
// h = an integer provided by the user.
...
#include "cs50.h"
// Comment out line below because you don't need this when you use the CS50 IDE
#include "cs50.c"
...
int main(void)
{
    // Declare variable which stores the result of calling the function mario(). 
    // Mario will return a value of 0 (successful) or 1 (unsuccessful)
    int result = mario();
...
// Declare function that takes userInput and calls another function which draws triangles
int mario()
{
    // Declare variable that will hold the user's input
    // Use get_int (from CS50 lib) to prompt user for an integer 
    // (if you use scanf("%d", &userInput), the program will accept decimals between 1 and 8).
...
// Helper function which draws triangle
int draw_triangle(int rows)
{
